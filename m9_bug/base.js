...
var Youhui = {
        ...
        if (module) {
            ...
            function(n, item) {
                ...
                if (!self._loadedModule.include(item)) {
                    self.include(self.RESOURCEURL + "/js/" + self.module[item] + ".js",
                    function() {
                        switch (item) {
                        case "core.form":
            ...
            if ($.browser.msie) {
                ...
                var checkInterval = setInterval(function() {
                    if ($("head>link").index(el) != -1) {
                        if (i < 10) clearInterval(checkInterval) if (typeof callback == 'function') callback();
                        i++;
                    }
                },
                200);
    ...
    api: { //AJAX接口调用
        ajax: function(type, action, data, callback, options) {
            if (action != undefined) var url = Youhui.SERVICEURL + action;
            else var url = location.pathname;
            $.ajax($.extend({
                url: url,
                data: data,
                async: true,
    ...
    dialog: { //模态框
        ...
        _createLayer: function() {
            ...
            _html.push('<div class="d_body" style="height:') _html.push(this.opt.height);
        ...
        resize: function(options) {
            if (options) {
                if (options.width) this._els.dialogBox.width(options.width);
                if (options.height) this._els.body.height(options.height);
            }
            if ($.browser.msie6) $(".maskIframe", this.pannel).height(this._els.dialogBox.height() - 4).width(this._els.dialogBox.width() - 4);
            if (this.opt.mask && !$.browser.msie6) this._els.dialogBox[0].style.position = "fixed";
            return this;
        },
        _regEvent: function() {
            ...
            $(window).resize(function() {
                ...
                if ($.browser.msie) { //ie resize bug
                    if ($(window).width() == this.windowSize.width && $(window).height() == this.windowSize.height) return;
                }
        ...
        setClassName: function(name, reset) {
            reset = reset || false;
            if (reset) this._els.dialogBox.attr("class", name);
            else this._els.dialogBox.addClass(name);
            return this;
        },
        ...
        setFoot: function(html) {
            if (this._els.foot && this._els.foot.length > 0) {
                ...
                if (html.constructor == Object) this._els.foot.append(html);
                else {
        ...
        setContent: function(html) {
            if (this._els.body && this._els.body.length > 0) {
                if (html.constructor == Object) this._els.content.append(html);
                else this._els.content.html(html);
            }
            this.setPos(this.opt.pos);
        ...
        show: function() {
            ...
            if (this.opt.mask) $(document).bind("DOMMouseScroll.dialog",
            function() {
                return false;
            });
        ...
        toggle: function() {
            if (this.hideStatus) return this.show();
        ...
        ajax: function(title, options) { //AJAX加载内容到窗口
            ...
            if (options.action) {
                ...
                _this._init();
                _this.show();
                Youhui.api.get(options.action, options.params || {},
                function(html) {
                    if (!this) return;
        ...
        bind: function(obj, cobj) {
            ...
            function() {
                ...
                if ($.inArray("none", _class) == -1 && $.inArray("more", _class) == -1) {
                    ...
                    if (tab_content.length > 0) tab_content.hide().eq(_index).show();
                }
            });
            //子选项卡
            $("ul.tabs_sub>li>a", obj).live("hover",
            function() {
    ...
    form: {
        bindDefault: function(obj) {
            $(obj || "input.default,textarea.default").live("focus",
            function() {
        ...
        bindFocus: function(obj) {
            $(obj || "input.text,textarea.textarea").live("focus",
            function() {
                $(this).addClass("focus");
            }).live("blur",
            function() {
    ...
    pager: { //分页控件
        ...
        bind: function(obj, options) {
            ...
            var _html = [];
            if (_this.opt.pagecount > 1 || _this.opt.total) _html.push('<div class="pager ' + (_this.opt.type == "numeric" ? "pager_numeric": "") + '">\n');
            if (_this.opt.total) {
            ...
            if (_this.opt.pagecount > 1 || _this.opt.total) _html.push('</div>');
    ...
    delayLoader: { //延迟加载
        init: function(options) {
            ...
            if (opt.event == "scroll") {
                $(opt.container).bind("scroll.loader resize.loader",
                function(event) {
...
jQuery.fn.extend({ //jQuery 扩展
    drag: function(position, target, offset) {
        ...
        return this.css("cursor", "move").bind("mousedown.drag",
        function(e) {
            e.preventDefault();
            e.stopPropagation();
            ...
            jQuery(document).unbind(".drag").bind("mousemove.drag",
            function(e) {
                var posX = e.pageX - target.posX,
                posY = e.pageY - target.posY;
                target.css({
                    left: function() {
                        if (posX > 0 && posX + width < mainW) return posX;
                        else if (posX <= 0) return offset.x;
                        else if (posX + width >= mainW) return mainW - width
                    },
                    top: function() {
                        if (posY > 0 && posY + height < mainH) return posY;
                        else if (posY <= 0) return offset.y;
                        else if (posY + height >= mainH) return mainH - height;
                    }
                });
            }).bind("mouseup.drag",
            function(e) {
